...
def main(page: ft.Page):
    ...
    # 💡 日本語エラーを絶対に起こさないPython内蔵の超軽量ハッシュ暗号化
    ...
    def handle_forgot_reset_password(e):
        ...
        try:
            ...
            if not res.data or res.data.get("secret_answer") != ans:
    ...
    # 🛠️ 動作軽量化：固定画面サイズの縦カラムからは重くなる原因のスクロール指定を全削除
    ...
    # 🛠️ 軽量＆安定化：ツールバーアイコンをノーマル仕様（確実に表示されるタイプ）に変更
        ...
        controls=[
            ...
            ft.Container(
                content=ft.Row([
                    ...
                    ft.Row([
                        ft.IconButton(icon=ft.Icons.ACCOUNT_CIRCLE, tooltip="名前変更", on_click=lambda e: open_mypage_dialog(change_name_dialog), icon_color=ft.Colors.BLUE_600),
                        ...
                        ft.IconButton(icon=ft.Icons.VISIBILITY, tooltip="ランキング公開設定", on_click=lambda e: open_mypage_dialog(privacy_setting_dialog), icon_color=ft.Colors.BLUE_600),
                        ft.IconButton(icon=ft.Icons.DELETE_FOREVER, tooltip="アカウントの完全削除", on_click=trigger_delete_confirmation, icon_color=ft.Colors.RED_600)
                    ], spacing=1)
    ...
    # 🛠️ 全画面フィット：ベースを「expand=True」で全画面引き伸ばし
