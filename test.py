...
def main(page: ft.Page):
    ...
    def handle_forgot_check_user(e):
        ...
        if not name:
            ...
            return
        try:
            res = supabase.table("users").select("secret_question").execute()
    ...
    def handle_forgot_reset_password(e):
        ...
        if len(new_p) < 4:
            ...
            return
        try:
            res = supabase.table("users").select("secret_answer").execute()
    ...
    # 💡【0.28.3完全適合】スマホでも絶対に見切れない＆文字がくっきり見えるマイページ画面
        ...
        ft.Container(
            content=ft.Column([
                ...
                ft.Row(
                    ...
                    wrap=True,  # 💡 画面幅に合わせて自動で美しく折り返す設定
                    spacing=8,
                    run_spacing=5,
    ...
    # 💡【重要・復活】消えてしまっていたランキングと全体の画面組み立て、および起動処理
